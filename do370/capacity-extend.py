...
#
# CHANGELOG
#   * Thu May 03 2021 Iván Chavero ichavero@redhat.com
#   * Mon Mar 22 2021 Iván Chavero ichavero@redhat.com
#   - original code
"""
Grading module for DO370 capacity-extend guided exercise (or lab).
...
class CapacityExtend(OpenShift):
    """
    Capacity Extend lab script for DO370
    """
    __LAB__ = "capacity-extend"
    ...
    def start(self):
        ...
        items = [
            ...
            {
                "label": "Check if the 'capacity-extend-ge' project does not exist",
                "task": self._check_ge_namespace,
                "fatal": True,
            },
            {
                "label": "Check if the 'postgresql-persistent-sc' template does not exist",
                "task": self._check_template,
                "fatal": True,
            },
            {
    ...
    def finish(self):
        ...
        items = [
            ...
            {
                "label": "Delete the capacity-extend-ge project",
    ...
    def _delete_ge_namespace(self, item):
        item["failed"] = False
        try:
            self.delete_resource("v1", "Namespace", "capacity-extend-ge", "")
        except Exception as e:
            item["failed"] = True
    ...
    def _delete_template(self, item):
        item["failed"] = False
        try:
            self.delete_resource("template.openshift.io/v1", "Template", "postgresql-persistent-sc", "openshift")
        except Exception as e:
            item["failed"] = True
    ...
    def _check_ge_namespace(self, item):
        ...
        """
        item["failed"] = False
        if self.resource_exists("v1", "Namespace", "capacity-extend-ge", ""):
            item["failed"] = True
            item["msgs"] = [{"text":
                "The capacity-extend-ge namespace already exists, please " +
                "delete it or run 'lab finish capacity-extend' " +
